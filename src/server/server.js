...
server
  .on("connect", (socket) =>
    log.debug(
      `Client ${socket.remoteAddress}:${socket.remotePort} connected to the server at ${socket.localAddress}:${socket.localPort}.`
    )
  )
  .on("connection", (socket) =>
    log.debug(
      `Connection established from ${socket.remoteAddress} port ${socket.remotePort} to ${socket.localAddress} port ${socket.localPort}`
    )
  )
  .on("request", (req, res) =>
    log.debug(`Request received. URL: ${req.url}. Method: ${req.method}`)
  )
  .on("upgrade", (req, socket, head) =>
    log.debug(`Upgrade requested. Header: ${head}\nSocket:${socket}`)
  );
